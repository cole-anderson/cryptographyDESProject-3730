/*
  ...
  PROGRAM ASSIGNMENT: DES
	implement the DES algorithm to encrypt or decrypt a textfiles contents and overwrite
*/
//LIBRARIES/NAMESPACE
#include<iostream>
#include<string>
...
void input()
{
  ...
  string fileName;
	...
	string outtoFile;
	int ftype;
	cout << "WARNING! THIS PROGRAM OVERWRITES THE SOURCE FILE WHEN ENCRYPTING AND DECRYPTING" << endl;
  ...
  if(selection == 1)
  {
    ...
    cin >> fileName;
		...
		string expE = "1100000010110111101010001101000001011111001110101000001010011100";
  ...
  else if(selection == 2)
  {
    ...
    cin >> fileName;
		...
		//Write to File
		write(outtoFile, fileName);
		string expD = "0001001000110100010101101010101111001101000100110010010100110110";
		cout << "Result:   " << outtoFile << endl;
...
string encryption(string key, string line)
{
  ...
  key = "1010101010111011000010010001100000100111001101101100110011011101";
	//SHOULD END UP AS: 1100000010110111101010001101000001011111001110101000001010011100
    ...
    right2 = line.substr(32,32);
		...
		for(int a = 0; a < 16; a++)
		{
			...
			for(int z = 0; z < 8; z++)
			{
				...
				int col = 8 * int(xxor[z * 6 + 1] - '0') + 4 * int(xxor[z * 6 + 2] - '0') + 2 * int(xxor[z * 6 + 3] - '0') + int(xxor[z * 6 + 4] - '0');
...
string decryption(string key,string line)
{
	...
	key = "1010101010111011000010010001100000100111001101101100110011011101";
	//SHOULD END UP AS: 0001001000110100010101101010101111001101000100110010010100110110
